...
export function EquipmentOverviewCard({ 
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "active":
        return <Badge variant="default" className="text-xs bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 border-0">{t('equipment.status.active')}</Badge>;
      case "maintenance":
        return <Badge variant="warning" className="text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border-0">{t('equipment.status.maintenance')}</Badge>;
      case "inactive":
        return <Badge variant="secondary" className="text-xs">{t('equipment.status.inactive')}</Badge>;
      default:
        return <Badge variant="secondary" className="text-xs">{t('equipment.status.unknown')}</Badge>;
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-semibold text-green-900 dark:text-green-100">
                {t('equipment.overview.title')}
              </CardTitle>
              <p className="text-sm text-green-600 dark:text-green-400">{t('equipment.overview.subtitle')}</p>
            </div>
          </div>
      ...
      <CardContent className="space-y-6 ml-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-4 bg-white/60 dark:bg-gray-800/40 backdrop-blur-sm rounded-xl border border-white/20 shadow-sm">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 font-medium mt-1">{t('equipment.overview.total')}</div>
          </div>
          <div className="text-center p-4 bg-green-50/80 dark:bg-green-900/30 backdrop-blur-sm rounded-xl border border-green-200/30 shadow-sm">
            ...
            <div className="text-xs text-green-600 dark:text-green-400 font-medium mt-1">{t('equipment.overview.active')}</div>
          </div>
          <div className="text-center p-4 bg-yellow-50/80 dark:bg-yellow-900/30 backdrop-blur-sm rounded-xl border border-yellow-200/30 shadow-sm">
            ...
            <div className="text-xs text-yellow-600 dark:text-yellow-400 font-medium mt-1">{t('equipment.overview.maintenance')}</div>
          </div>
        </div>
        
        {/* Equipment list */}
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">{t('equipment.overview.recent_equipment')}</h4>
          ...
          <div className="p-4 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/20 dark:to-orange-900/20 rounded-xl border border-yellow-200/40 dark:border-yellow-800/40 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-yellow-800 dark:text-yellow-200">{t('equipment.overview.upcoming_maintenance')}</div>
                <div className="text-xs text-yellow-600 dark:text-yellow-400">
                  {t('equipment.overview.equipment_requires_attention', { count: maintenanceCount })}
                </div>
              </div>
