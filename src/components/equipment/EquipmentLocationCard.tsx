...
export function EquipmentLocationCard({ 
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "moving":
        return <Badge variant="default" className="text-xs bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400">{t('tracking.moving')}</Badge>;
      case "parked":
        return <Badge variant="default" className="text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">{tCommon('parked')}</Badge>;
      case "offline":
        return <Badge variant="secondary" className="text-xs">{tCommon('offline')}</Badge>;
      default:
        return <Badge variant="secondary" className="text-xs">{tCommon('unknown')}</Badge>;
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg font-semibold text-blue-900 dark:text-blue-100">
                {t('tracking.fleet_tracking')}
              </CardTitle>
              <p className="text-sm text-blue-600 dark:text-blue-400">{t('tracking.real_time_locations')}</p>
            </div>
          </div>
      ...
      <CardContent className="space-y-6 ml-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-4 bg-green-50/80 dark:bg-green-900/30 backdrop-blur-sm rounded-xl border border-green-200/30 shadow-sm">
            ...
            <div className="text-xs text-green-600 dark:text-green-400 font-medium mt-1">{t('tracking.online')}</div>
          </div>
          <div className="text-center p-4 bg-blue-50/80 dark:bg-blue-900/30 backdrop-blur-sm rounded-xl border border-blue-200/30 shadow-sm">
            ...
            <div className="text-xs text-blue-600 dark:text-blue-400 font-medium mt-1">{t('tracking.moving')}</div>
          </div>
        </div>
