...
export function ExpenseTypesTab() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      // Check if there are any templates using this expense type
      ...
      // Check if there are any expense instances using this expense type
      ...
      if (instances && instances.length > 0) {
        throw new Error(
          `No se puede eliminar este tipo de gasto porque está siendo usado en ${instances.length} instancia(s) de gasto(s) registrada(s). Elimine primero los gastos o cambie su tipo.`
        );
      }
