...
import { useCreateDispatcherOtherIncome } from "@/hooks/useDispatcherOtherIncome";
...
export function CreateDispatcherOtherIncomeForm({ onClose }: { onClose: () => void }) {
  const { user } = useAuth();
  const { data: companyDispatchers = [], isLoading: dispatchersLoading } = useCompanyDispatchers();
  const createDispatcherOtherIncome = useCreateDispatcherOtherIncome();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.dispatcher_user_id || !formData.income_date) {
      console.error("Dispatcher or date not selected");
      return;
    }

    try {
      await createDispatcherOtherIncome.mutateAsync({
        dispatcher_user_id: formData.dispatcher_user_id,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <Label htmlFor="dispatcher">Despachador *</Label>
        <Select 
          value={formData.dispatcher_user_id} 
          onValueChange={(value) => setFormData({ ...formData, dispatcher_user_id: value })}
        >
          <SelectTrigger>
          ...
          <SelectContent>
            {dispatchersLoading ? (
              <SelectItem value="loading" disabled>Cargando despachadores...</SelectItem>
            ) : companyDispatchers.length === 0 ? (
              <SelectItem value="no-dispatchers" disabled>No hay despachadores disponibles</SelectItem>
            ) : (
              companyDispatchers.map((dispatcher) => (
                <SelectItem key={dispatcher.user_id} value={dispatcher.user_id}>
                  {`${dispatcher.first_name || ''} ${dispatcher.last_name || ''}`.trim() || dispatcher.user_id}
                </SelectItem>
              ))
            )}
          </SelectContent>
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        <Button 
          type="submit" 
          disabled={createDispatcherOtherIncome.isPending || !formData.dispatcher_user_id || !formData.income_date || !formData.description || !formData.income_type}
        >
          {createDispatcherOtherIncome.isPending ? (
            <>
