...
export default function Equipment() {
  ...
  const mockEquipmentData = equipment?.map(item => ({
    ...
    make: item.make || tCommon('unknown'),
    model: item.model || tCommon('unknown'),
    status: item.status as "active" | "maintenance" | "inactive",
    mileage: item.current_mileage,
    nextMaintenance: t('maintenance.next_maintenance', 'Next: 2024-08-15')
  })) || [];
  ...
  const mockLocationData = equipment?.slice(0, 5).map((item, index) => ({
    ...
    lastUpdate: tCommon('minutes_ago', { minutes: 2 }),
  ...
  const mockMaintenanceData = equipment?.slice(0, 4).map((item, index) => ({
    id: item.id,
    equipmentNumber: item.equipment_number,
    maintenanceType: index % 3 === 0 ? t('maintenance.types.annual_inspection') : index % 3 === 1 ? t('maintenance.types.oil_change') : t('maintenance.types.general_inspection'),
                ...
                {isLoading ? (
                  <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                      ...
                      <p className="text-muted-foreground">{tCommon("loading", "Cargando...")}</p>
                    </div>
                  </div>
                ) : error ? (
                  <div className="flex items-center justify-center h-64">
                    <div className="text-center">
                      <p className="text-destructive mb-2">{tCommon("error", "Error al cargar los datos")}</p>
