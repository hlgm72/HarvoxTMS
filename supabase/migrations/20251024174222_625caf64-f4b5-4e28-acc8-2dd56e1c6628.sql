-- Fix expense_instances foreign key violation in generate_recurring_expense_instances_on_template
-- The function was using user_payroll_id for payment_period_id, but expense_instances.payment_period_id
-- now references company_payment_periods.id directly after migration 20251013003225
...
BEGIN
  ...
  -- Si es UPDATE, eliminar instancias y guardar payrolls afectados para recalcular
  IF TG_OP = 'UPDATE' THEN
    -- Guardar lista de payrolls afectados ANTES de eliminar
    -- ✅ FIXED: Now using company_payment_period_id correctly
    ...
    -- Eliminar instancias modificables del template específico
    ...
    -- Recalcular payrolls afectados después de eliminar
  ...
  WHILE v_period_date <= v_end_date LOOP
    ...
    -- Para frecuencia mensual, verificar month_week
    IF NEW.frequency = 'monthly' AND NEW.month_week IS NOT NULL THEN
      ...
      -- Verificar si la semana del período coincide con la semana configurada
    ...
    -- ✅ FIXED: Now using v_company_payment_period_id instead of v_user_payroll_id
    IF NOT EXISTS (
      ...
      -- ✅ FIXED: Now inserting v_company_payment_period_id instead of v_user_payroll_id
...
'Genera instancias de gastos recurrentes usando company_payment_period_id directamente en expense_instances';