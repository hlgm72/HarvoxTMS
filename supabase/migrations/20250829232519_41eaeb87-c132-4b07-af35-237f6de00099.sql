-- Crear función ACID para operaciones de cargas (loads)
...
BEGIN
  ...
  -- Extraer company_id de los datos
  ...
  IF (load_data->>'id') IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM loads
      WHERE id = load_id AND company_id = target_company_id
    ) THEN
  ...
  ELSE
    -- CREACIÓN
    -- Verificar que el load_number no existe en la empresa
    IF EXISTS (
      SELECT 1 FROM loads
      WHERE load_number = load_data->>'load_number'
      AND company_id = target_company_id
    ) THEN
  ...
  RETURN jsonb_build_object(
    ...
    END,
    'operation', CASE 
      WHEN (load_data->>'id') IS NOT NULL THEN 'UPDATE'
      ELSE 'CREATE'
    END,
