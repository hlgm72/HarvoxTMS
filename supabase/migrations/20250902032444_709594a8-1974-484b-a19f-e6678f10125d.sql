-- Fix column ambiguity in simple_load_operation_with_deductions function
...
DECLARE
  ...
  load_total_amount NUMERIC;  -- Renamed to avoid ambiguity
BEGIN
  ...
  IF result_load.driver_user_id IS NOT NULL THEN
    ...
    IF result_load.factoring_fee_percentage > 0 THEN
      factoring_amount := (load_total_amount * result_load.factoring_fee_percentage / 100);
    END IF;
    
    IF result_load.dispatching_fee_percentage > 0 THEN
      dispatching_amount := (load_total_amount * result_load.dispatching_fee_percentage / 100);
    END IF;
    
    IF result_load.leasing_fee_percentage > 0 THEN
      leasing_amount := (load_total_amount * result_load.leasing_fee_percentage / 100);
    END IF;
