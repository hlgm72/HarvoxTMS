-- Fix deduction calculation linking in recalculate function
...
BEGIN
  ...
  -- Get the driver calculation record with company info
  SELECT 
    dpc.*,
    cpp.company_id
  INTO driver_calc
  ...
  WHERE l.driver_user_id = driver_calc.driver_user_id
    AND l.payment_period_id = driver_calc.company_payment_period_id;
  ...
  WHERE fe.driver_user_id = driver_calc.driver_user_id
    AND fe.payment_period_id = driver_calc.company_payment_period_id;

  -- 🚨 CORRECCIÓN CRÍTICA: expense_instances.payment_period_id = calculation_id
  -- Calculate deductions (the payment_period_id in expense_instances is the calculation ID)
  SELECT 
    COALESCE(SUM(ei.amount), 0) as total_deductions
  INTO deduction_totals
  FROM expense_instances ei
  JOIN expense_types et ON ei.expense_type_id = et.id
  WHERE ei.payment_period_id = calculation_id  -- This is the key fix!
    AND ei.user_id = driver_calc.driver_user_id
    AND et.category IN ('deduction', 'percentage_deduction', 'fixed_deduction')
    AND ei.status = 'applied';

  -- Calculate other income (solo income, no deductions)
  SELECT 
  ...
  RAISE NOTICE '🔧 Driver recalc FIXED - ID: %, Gross: %, Fuel: %, Deductions: %, Income: %, Net: %', 
    calculation_id, 
    COALESCE(load_totals.gross_earnings, 0),
    COALESCE(fuel_totals.fuel_expenses, 0),
    COALESCE(deduction_totals.total_deductions, 0),
    COALESCE(other_income_totals.other_income, 0),
    final_net_payment;
