-- Crear la función que falta para recalcular totales de períodos de pago
CREATE OR REPLACE FUNCTION public.recalculate_payment_period_totals(period_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  calculation_record RECORD;
  total_gross numeric := 0;
  total_fuel numeric := 0;
  total_deductions numeric := 0;
  total_other_income numeric := 0;
  total_income numeric := 0;
  total_net numeric := 0;
BEGIN
  -- Recalcular totales para cada conductor en este período
  FOR calculation_record IN 
    SELECT id FROM driver_period_calculations 
    WHERE id = period_id OR company_payment_period_id = period_id
  LOOP
    -- Calcular deducciones totales para este conductor/período
    SELECT COALESCE(SUM(amount), 0) INTO total_deductions
    FROM expense_instances
    WHERE payment_period_id = calculation_record.id
    AND status = 'applied';
    
    -- Obtener otros valores del cálculo actual
    SELECT 
      COALESCE(gross_earnings, 0),
      COALESCE(fuel_expenses, 0),
      COALESCE(other_income, 0)
    INTO total_gross, total_fuel, total_other_income
    FROM driver_period_calculations
    WHERE id = calculation_record.id;
    
    -- Calcular ingresos totales y pago neto
    total_income := total_gross + total_other_income;
    total_net := total_income - total_fuel - total_deductions;
    
    -- Actualizar el registro de cálculos del conductor
    UPDATE driver_period_calculations
    SET 
      total_deductions = recalculate_payment_period_totals.total_deductions,
      total_income = recalculate_payment_period_totals.total_income,
      net_payment = recalculate_payment_period_totals.total_net,
      has_negative_balance = (recalculate_payment_period_totals.total_net < 0),
      updated_at = now()
    WHERE id = calculation_record.id;
  END LOOP;
  
  RAISE LOG 'Recalculated totals for period %', period_id;
END;
$function$;