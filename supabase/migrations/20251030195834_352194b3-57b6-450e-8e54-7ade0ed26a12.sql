-- Fix: Disable cleanup triggers during load deletion to avoid conflicts
...
BEGIN
  ...
  -- CRITICAL: Disable cleanup triggers to avoid conflicts
  -- (our function handles all cleanup logic)
  ALTER TABLE expense_instances DISABLE TRIGGER trigger_cleanup_empty_period_after_deduction_delete;
  ALTER TABLE expense_instances DISABLE TRIGGER trigger_cleanup_payroll_after_expense_delete;
      ...
      IF NOT EXISTS (
        ...
        IF company_period_id IS NOT NULL THEN
          ...
          IF NOT period_has_any_payrolls THEN
            -- Delete expense_instances using EXECUTE to bypass RLS
  ...
  EXCEPTION
    WHEN OTHERS THEN
      -- Re-enable triggers before re-raising
      ALTER TABLE expense_instances ENABLE TRIGGER trigger_cleanup_empty_period_after_deduction_delete;
      ALTER TABLE expense_instances ENABLE TRIGGER trigger_cleanup_payroll_after_expense_delete;
      RAISE;
  END;

  -- Re-enable triggers after successful completion
  ALTER TABLE expense_instances ENABLE TRIGGER trigger_cleanup_empty_period_after_deduction_delete;
  ALTER TABLE expense_instances ENABLE TRIGGER trigger_cleanup_payroll_after_expense_delete;
