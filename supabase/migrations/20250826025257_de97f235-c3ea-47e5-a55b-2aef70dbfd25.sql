-- Continue refactoring more critical functions

-- 5. Update create_or_update_company_with_validation function
...
BEGIN
  ...
  IF operation_type = 'UPDATE' THEN
    ...
    IF NOT is_superadmin AND NOT EXISTS (
      ...
      RAISE EXCEPTION 'ERROR_NO_PERMISSIONS_MANAGE_COMPANY';
    END IF;
  ...
  ELSE
    ...
    IF NOT is_superadmin THEN
      RAISE EXCEPTION 'ERROR_NO_PERMISSIONS_CREATE_COMPANIES';
    END IF;
  END IF;
  ...
  IF NULLIF(company_data->>'street_address', '') IS NULL THEN
    RAISE EXCEPTION 'ERROR_STREET_ADDRESS_REQUIRED';
  END IF;

  IF NULLIF(company_data->>'state_id', '') IS NULL THEN
    RAISE EXCEPTION 'ERROR_STATE_ID_REQUIRED';
  END IF;

  IF NULLIF(company_data->>'zip_code', '') IS NULL THEN
    RAISE EXCEPTION 'ERROR_ZIP_CODE_REQUIRED';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'ERROR_COMPANY_NAME_EXISTS:name:%', company_data->>'name';
  END IF;
  ...
  IF NULLIF(company_data->>'dot_number', '') IS NOT NULL THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION 'ERROR_DOT_NUMBER_EXISTS:number:%', company_data->>'dot_number';
    END IF;
  END IF;
  ...
  IF NULLIF(company_data->>'mc_number', '') IS NOT NULL THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION 'ERROR_MC_NUMBER_EXISTS:number:%', company_data->>'mc_number';
    END IF;
  END IF;
  ...
  -- ================================
  -- 4. HANDLE OWNER DETAILS (if provided)
  -- ================================
  
  IF (company_data ? 'owner_name') OR (company_data ? 'owner_email') OR (company_data ? 'owner_phone') OR (company_data ? 'owner_title') THEN
    INSERT INTO company_owner_details (
      company_id,
      owner_name,
      owner_email,
      owner_phone,
    ...
    ON CONFLICT (company_id) DO UPDATE SET
      owner_name = COALESCE(EXCLUDED.owner_name, company_owner_details.owner_name),
      owner_email = COALESCE(EXCLUDED.owner_email, company_owner_details.owner_email),
      owner_phone = COALESCE(EXCLUDED.owner_phone, company_owner_details.owner_phone),
      owner_title = COALESCE(EXCLUDED.owner_title, company_owner_details.owner_title),
      updated_at = now();
  END IF;

  -- ================================
  -- 5. GET COMPLETE COMPANY DATA INCLUDING OWNER DETAILS
  -- ================================
  ...
  SELECT 
    ...
    cod.owner_phone,
    cod.owner_title
  INTO complete_company_data
  FROM companies c
  LEFT JOIN company_owner_details cod ON c.id = cod.company_id
  WHERE c.id = result_company.id;

  -- Return success result with complete company data including owner details
  RETURN jsonb_build_object(
    ...
    END,
    'company', row_to_json(complete_company_data),
