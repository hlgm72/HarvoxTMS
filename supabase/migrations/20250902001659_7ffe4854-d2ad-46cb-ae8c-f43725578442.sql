-- Corregir función simple_load_operation para no usar company_id en tabla loads
-- En su lugar, obtener company_id a través del driver_user_id
...
BEGIN
  ...
  IF array_length(stops_data, 1) > 0 THEN
    ...
    FOR stop_data IN SELECT unnest(stops_data) LOOP
      ...
      result_stops := result_stops || stop_result::JSONB;
    END LOOP;
  END IF;
