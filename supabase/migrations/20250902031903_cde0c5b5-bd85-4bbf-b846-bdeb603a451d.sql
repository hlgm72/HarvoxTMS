-- Fix deduction descriptions by directly updating existing percentage deductions
-- This will change "Factoring deduction for load X" to "Factoring fees"
...
BEGIN
  ...
  IF result_load.driver_user_id IS NOT NULL THEN
    ...
    SELECT et.id INTO factoring_expense_type_id FROM expense_types et WHERE et.category = 'percentage_deduction' AND et.name = 'Factoring' LIMIT 1;
    SELECT et.id INTO dispatching_expense_type_id FROM expense_types et WHERE et.category = 'percentage_deduction' AND et.name = 'Dispatching' LIMIT 1;
    SELECT et.id INTO leasing_expense_type_id FROM expense_types et WHERE et.category = 'percentage_deduction' AND et.name = 'Leasing' LIMIT 1;

    -- Get driver period calculation ID
    SELECT dpc.id INTO driver_period_calc_id
    ...
    IF driver_period_calc_id IS NOT NULL THEN
      -- FACTORING DEDUCTION - Update or Insert
      IF result_load.factoring_percentage > 0 AND factoring_expense_type_id IS NOT NULL THEN
        ...
        IF EXISTS (
          ...
          WHERE payment_period_id = driver_period_calc_id 
          AND user_id = result_load.driver_user_id 
          AND expense_type_id = factoring_expense_type_id
        ) THEN
          UPDATE expense_instances SET
            amount = (result_load.total_amount * result_load.factoring_percentage / 100),
            description = 'Factoring fees',
            expense_date = load_pickup_date,
            updated_at = now()
          WHERE payment_period_id = driver_period_calc_id 
          AND user_id = result_load.driver_user_id 
          AND expense_type_id = factoring_expense_type_id;
      ...
      -- DISPATCHING DEDUCTION - Update or Insert
      IF result_load.dispatching_percentage > 0 AND dispatching_expense_type_id IS NOT NULL THEN
        ...
        IF EXISTS (
          ...
          UPDATE expense_instances SET
            amount = (result_load.total_amount * result_load.dispatching_percentage / 100),
      ...
      -- LEASING DEDUCTION - Update or Insert
      IF result_load.leasing_percentage > 0 AND leasing_expense_type_id IS NOT NULL THEN
        ...
        IF EXISTS (
          ...
          UPDATE expense_instances SET
            amount = (result_load.total_amount * result_load.leasing_percentage / 100),
