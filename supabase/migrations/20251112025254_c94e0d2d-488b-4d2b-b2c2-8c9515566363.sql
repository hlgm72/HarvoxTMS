-- Corregir función eliminando uploaded_at que no existe en la tabla
...
COMMENT ON FUNCTION get_load_documents_with_validation IS 'Returns load documents ordered chronologically (oldest first, newest last). Fixed: removed uploaded_at column that does not exist.';