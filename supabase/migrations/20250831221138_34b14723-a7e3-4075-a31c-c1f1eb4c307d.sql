-- Actualizar función RPC para incluir station_city en las operaciones de fuel_expenses
CREATE OR REPLACE FUNCTION create_or_update_fuel_expense_with_validation(expense_data jsonb, expense_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- For UPDATE operations, validate expense exists
  ...
  IF operation_type = 'CREATE' THEN
    INSERT INTO fuel_expenses (
      ...
      NULLIF(expense_data->>'station_city', ''),
  ...
  ELSE
    UPDATE fuel_expenses SET
      ...
      station_city = NULLIF(expense_data->>'station_city', ''),
