-- Fix the create_other_income_with_validation function
-- The issue is with variable scoping and incorrect period ID usage
...
BEGIN
  ...
  -- Ensure driver calculation exists for this period
  IF NOT EXISTS (
  ...
  INSERT INTO other_income (
    ...
    target_period_id,  -- Use target_period_id (company_payment_period.id) not calculation_period_id
  ...
  -- ================================
  PERFORM calculate_driver_payment_period_v2(
    (income_data->>'user_id')::UUID,
    target_period_id
  );
