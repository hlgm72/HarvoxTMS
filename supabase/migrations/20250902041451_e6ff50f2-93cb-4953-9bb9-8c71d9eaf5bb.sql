-- First, clean up the duplicated deductions 
-- Delete the incorrect doubled amounts and recalculate from scratch

-- Clean up the doubled percentage deductions for the recent period
...
-- Now fix the function to calculate correctly without doubling
...
BEGIN
  ...
  -- Handle percentage deductions ONLY if driver is assigned
  IF result_load.driver_user_id IS NOT NULL AND payment_period_id_result IS NOT NULL THEN
    ...
    IF driver_calculation_id IS NOT NULL THEN
      ...
      -- Calculate amounts for THIS SPECIFIC LOAD
      ...
      -- Create individual expense instances per load (no ON CONFLICT - let them accumulate naturally)
      IF factoring_amount > 0 AND factoring_expense_type_id IS NOT NULL THEN
        INSERT INTO expense_instances (
          ...
          factoring_amount, 'Factoring fee - Load ' || result_load.load_number,
      ...
      IF dispatching_amount > 0 AND dispatching_expense_type_id IS NOT NULL THEN
        INSERT INTO expense_instances (
          ...
          dispatching_amount, 'Dispatching fee - Load ' || result_load.load_number,
          result_load.pickup_date, 'applied', now(), current_user_id
        );
      END IF;
      ...
      IF leasing_amount > 0 AND leasing_expense_type_id IS NOT NULL THEN
        INSERT INTO expense_instances (
          ...
          leasing_amount, 'Leasing fee - Load ' || result_load.load_number,
          result_load.pickup_date, 'applied', now(), current_user_id
        );
      END IF;
    END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE WHEN operation_type = 'CREATE' THEN 'Carga creada exitosamente' ELSE 'Carga actualizada exitosamente' END,
