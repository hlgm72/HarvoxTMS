-- Enhance company data security by implementing stricter RLS policies
-- and improved audit logging to prevent business data theft

-- First, update the SELECT policy to be more restrictive
-- Only allow access through secure RPC functions
DROP POLICY IF EXISTS "companies_secure_select_members_only" ON public.companies;

-- Create extremely restrictive SELECT policy - force use of secure RPC functions
...
-- Update other policies to use optimized auth function calls
DROP POLICY IF EXISTS "companies_secure_update_owners_only" ON public.companies;
DROP POLICY IF EXISTS "companies_secure_insert_superadmin_only" ON public.companies;
...
USING (
  (SELECT auth.uid()) IS NOT NULL 
  AND NOT COALESCE(((SELECT auth.jwt())->>'is_anonymous')::boolean, false)
  AND (user_is_company_owner(id) OR user_is_superadmin())
)
WITH CHECK (
  (SELECT auth.uid()) IS NOT NULL 
  AND NOT COALESCE(((SELECT auth.jwt())->>'is_anonymous')::boolean, false)
...
-- Update the secure RPC functions to include enhanced logging
