-- Fix: Eliminar TODOS los expense_instances del company period antes de eliminarlo
  ...
  IF user_payroll_id IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      ...
      IF company_period_id IS NOT NULL THEN
        ...
        -- If no payrolls left, delete ALL related records and then the period
        IF NOT period_has_any_payrolls THEN
          -- CRITICAL: Delete ALL expense_instances that reference this company period
          -- (payment_period_id in expense_instances points to company_payment_periods.id)
          ...
          -- Now safe to delete the company period (no more FKs pointing to it)
