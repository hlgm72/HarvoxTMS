-- Función para asignar automáticamente período de pago basado en criterio de empresa
...
-- Actualizar función de operación de carga para usar el criterio de empresa
CREATE OR REPLACE FUNCTION simple_load_operation(operation_data jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Buscar período de pago apropiado si tenemos fecha objetivo
  IF target_date IS NOT NULL THEN
    SELECT id INTO matching_period_id
    FROM company_payment_periods
  ...
  ELSE
    ...
    UPDATE loads SET
      client_id = COALESCE(NULLIF(operation_data->>'client_id', '')::UUID, client_id),
  ...
  RETURN jsonb_build_object(
    ...
    'payment_period_assigned', matching_period_id IS NOT NULL,
    'assignment_criteria_used', company_criteria,
