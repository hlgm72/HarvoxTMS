-- Fix ambiguous column reference in simple_load_operation_with_deductions function
-- Add table aliases to resolve load_id column ambiguity
...
BEGIN
  ...
  ELSE
    UPDATE loads SET
      ...
      internal_dispatcher_id = COALESCE((load_data->>'internal_dispatcher_id')::UUID, internal_dispatcher_id),
      po_number = COALESCE(load_data->>'po_number', po_number),
      customer_name = COALESCE(load_data->>'customer_name', customer_name),
      client_id = COALESCE((load_data->>'client_id')::UUID, client_id),
      client_contact_id = COALESCE((load_data->>'client_contact_id')::UUID, client_contact_id),
  ...
  IF jsonb_array_length(stops_data) > 0 THEN
    ...
    IF operation_type = 'UPDATE' THEN
      DELETE FROM load_stops WHERE load_stops.load_id = load_id;
    END IF;
