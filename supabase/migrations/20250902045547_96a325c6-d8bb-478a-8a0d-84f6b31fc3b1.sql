-- Fix duplicate constraint violation in automatic deduction creation
-- Check if deductions already exist before creating them
...
BEGIN
  ...
  IF (load_data->>'driver_user_id')::UUID IS NOT NULL AND load_total_amount > 0 THEN
    ...
    IF driver_calculation_id IS NOT NULL THEN
      ...
      -- ✅ FIXED: Create or update deductions to avoid duplicates
