-- Fix ambiguous column reference "total_deductions" in calculate_driver_payment_period_with_validation
-- Rename local variable total_deductions to calculated_total_deductions to avoid conflict with column name
...
DECLARE
  ...
  calculated_total_deductions NUMERIC := 0;  -- FIX: Renamed from total_deductions to avoid ambiguity
...
BEGIN
  ...
  -- Calculate deductions from expense instances - FIX: Use renamed variable
  ...
  -- Calculate net payment - FIX: Use renamed variable
  calculated_net_payment := total_gross_earnings + total_other_income - total_fuel_expenses - calculated_total_deductions;
  ...
  -- Update the calculation record - FIX: Use column name explicitly with renamed variable
  UPDATE driver_period_calculations
  SET 
    gross_earnings = total_gross_earnings,
    fuel_expenses = total_fuel_expenses,
    other_income = total_other_income,
    total_deductions = calculated_total_deductions,  -- Column name = local variable (renamed)
