-- Corregir todas las funciones que usan fe.amount por fe.total_amount
...
BEGIN
  ...
  -- Calculate fuel expenses (CORREGIDO - usar total_amount)
  SELECT 
    COALESCE(SUM(fe.total_amount), 0) as fuel_expenses
  INTO fuel_totals
...
-- También corregir la función recalculate_payment_period_totals si existe
...
DECLARE
  current_user_id UUID;
  company_info RECORD;
  calculation_cursor CURSOR FOR 
    SELECT dpc.id as calculation_id, dpc.driver_user_id
    FROM driver_period_calculations dpc
...
BEGIN
  ...
  -- Get company info for this period
  ...
  IF NOT EXISTS (
    SELECT 1 FROM user_company_roles
    WHERE user_id = current_user_id
    AND company_id = company_info.company_id
    AND role IN ('company_owner', 'operations_manager', 'superadmin')
    AND is_active = true
  ) THEN
    RAISE EXCEPTION 'Sin permisos para recalcular este período';
  END IF;

  -- Iterate through each driver calculation in this period
  FOR calc_record IN calculation_cursor LOOP
    -- Calculate load totals for this driver in this period
    SELECT 
      COALESCE(SUM(l.total_amount), 0) as gross_earnings
    INTO load_totals
    ...
    WHERE l.driver_user_id = calc_record.driver_user_id
      AND l.payment_period_id = calc_record.calculation_id;
    ...
    WHERE fe.driver_user_id = calc_record.driver_user_id
      AND fe.payment_period_id = calc_record.calculation_id;
    ...
    FROM expense_instances ei
    JOIN expense_types et ON ei.expense_type_id = et.id
    WHERE ei.payment_period_id = calc_record.calculation_id
      AND ei.user_id = calc_record.driver_user_id
      AND et.category = 'deduction'
      AND ei.status = 'applied';
    ...
    UPDATE driver_period_calculations SET
      ...
      calculated_by = current_user_id,
      updated_at = now()
    WHERE id = calc_record.calculation_id;
  ...
  RETURN jsonb_build_object(
    ...
    'recalculated_by', current_user_id,
    'recalculated_at', now()
  );
