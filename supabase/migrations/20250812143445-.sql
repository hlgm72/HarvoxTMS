-- Check if the create_or_update_load_with_validation function exists and identify the issue
-- The error suggests there's a stack depth limit exceeded, which typically means infinite recursion

-- Let's see the current function definition first
SELECT pg_get_functiondef(oid) 
FROM pg_proc 
WHERE proname = 'create_or_update_load_with_validation';

-- For debugging purposes, let's also create a simplified version that avoids potential recursion
-- First, let's create a new working function without the problematic recursion
...
BEGIN
  ...
  -- Check for duplicate load numbers within company (exclude current load if editing)
  IF EXISTS (
    SELECT 1 FROM loads
    WHERE company_id = target_company_id
    AND load_number = load_data->>'load_number'
    AND (operation_type = 'create' OR id != existing_load_id)
  ) THEN
