-- Fix the ON CONFLICT logic to REPLACE amounts instead of adding them
...
DECLARE
  ...
  existing_factoring NUMERIC := 0;
  existing_dispatching NUMERIC := 0;
  existing_leasing NUMERIC := 0;
BEGIN
  ...
  IF result_load.driver_user_id IS NOT NULL AND payment_period_id_result IS NOT NULL THEN
    ...
    IF driver_calculation_id IS NOT NULL THEN
      ...
      -- Get existing amounts to ADD to (not replace)
      SELECT COALESCE(SUM(CASE WHEN et.name ILIKE '%factoring%' THEN ei.amount ELSE 0 END), 0) INTO existing_factoring
      ...
      SELECT COALESCE(SUM(CASE WHEN et.name ILIKE '%dispatch%' THEN ei.amount ELSE 0 END), 0) INTO existing_dispatching
      FROM expense_instances ei
      JOIN expense_types et ON ei.expense_type_id = et.id
      WHERE ei.payment_period_id = driver_calculation_id
      AND et.category = 'percentage_deduction';

      SELECT COALESCE(SUM(CASE WHEN et.name ILIKE '%leas%' THEN ei.amount ELSE 0 END), 0) INTO existing_leasing
      FROM expense_instances ei
      JOIN expense_types et ON ei.expense_type_id = et.id
      WHERE ei.payment_period_id = driver_calculation_id
      AND et.category = 'percentage_deduction';

      -- Create or update expense instances (FIXED: Replace total amount, not add to existing)
      IF factoring_amount > 0 AND factoring_expense_type_id IS NOT NULL THEN
        ...
        DO UPDATE SET
          -- FIXED: Set to existing amount + new amount (correct accumulation)
          amount = existing_factoring + factoring_amount,
      ...
      IF dispatching_amount > 0 AND dispatching_expense_type_id IS NOT NULL THEN
        ...
        DO UPDATE SET
          -- FIXED: Set to existing amount + new amount (correct accumulation)
          amount = existing_dispatching + dispatching_amount,
      ...
      IF leasing_amount > 0 AND leasing_expense_type_id IS NOT NULL THEN
        ...
        DO UPDATE SET
          -- FIXED: Set to existing amount + new amount (correct accumulation)
          amount = existing_leasing + leasing_amount,
