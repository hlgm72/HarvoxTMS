-- Fix ambiguous column references by renaming variables
...
BEGIN
  ...
  target_transaction_date := (expense_data->>'transaction_date')::DATE;
  ...
  SELECT id INTO target_payment_period_id
  FROM company_payment_periods
  WHERE company_id = target_company_id
    AND period_start_date <= target_transaction_date
    AND period_end_date >= target_transaction_date
    AND status IN ('open', 'processing')
  LIMIT 1;

  -- If no existing period found, create one
  IF target_payment_period_id IS NULL THEN
    target_payment_period_id := create_payment_period_if_needed(target_company_id, target_transaction_date);
  END IF;
  
  -- Final validation that we have a valid payment period
  IF target_payment_period_id IS NULL THEN
    RAISE EXCEPTION 'ERROR_OPERATION_FAILED: Could not find or create payment period for date %', target_transaction_date;
  END IF;
