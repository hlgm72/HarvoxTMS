-- Fix the ambiguous column reference in simple_load_operation function
...
BEGIN
  ...
  IF operation_mode = 'edit' THEN
    ...
    -- Delete existing stops for this load (use explicit table name to avoid ambiguity)
    DELETE FROM public.load_stops WHERE public.load_stops.load_id = load_id;
  ...
  -- Insert stops (be explicit with table names)
