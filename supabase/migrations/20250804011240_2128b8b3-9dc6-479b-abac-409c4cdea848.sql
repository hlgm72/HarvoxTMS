-- Fix security functions by dropping all dependent policies first, then recreating everything

-- Drop all policies that depend on the security functions
...
-- Now drop and recreate the security functions
DROP FUNCTION IF EXISTS public.is_authenticated_non_anon() CASCADE;
DROP FUNCTION IF EXISTS public.get_current_user_id_optimized() CASCADE;  
DROP FUNCTION IF EXISTS public.is_user_superadmin_safe() CASCADE;
DROP FUNCTION IF EXISTS public.is_user_admin_in_company_safe() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_company_ids_safe() CASCADE;
...
-- Recreate equipment policies with proper auth checks
...
-- Recreate expense_recurring_templates policies with strict auth checks
...
-- Recreate other_income policies with strict auth checks
