-- Actualizar la función simple_load_operation para usar 'created' como estado por defecto
-- en lugar de 'draft'
...
BEGIN
  ...
  IF operation_type = 'CREATE' THEN
    INSERT INTO public.loads (
      ...
      COALESCE(load_data->>'status', 'created'), -- ✅ CAMBIADO: 'created' en lugar de 'draft'
  ...
  RETURN jsonb_build_object(
    ...
    'load_number', result_load.load_number,
    'status', result_load.status,
    'message', CASE 
      WHEN operation_type = 'UPDATE' THEN 'Carga actualizada exitosamente'
...
-- También actualizar las cargas existentes que están en 'draft' a 'created'
UPDATE public.loads 
SET status = 'created', updated_at = now()
WHERE status = 'draft';

-- Actualizar el valor por defecto en la tabla para futuras cargas
