...
-- ============================================
-- 🔧 CORRECCIÓN: mark_driver_as_paid debe actualizar loads 'pending' y 'approved'
-- ============================================
-- PROBLEMA: La función solo actualizaba loads con payment_status = 'approved'
-- SOLUCIÓN: Actualizar también loads con payment_status = 'pending'

-- 1. Primero arreglar las cargas existentes que están en estado incorrecto
UPDATE loads l
SET 
...
WHERE up.user_id = l.driver_user_id
  ...
  AND l.payment_status IN ('pending', 'approved');

-- 2. Actualizar la función mark_driver_as_paid_with_validation
...
BEGIN
  ...
  -- ✅ CORRECCIÓN: Marcar loads como 'applied' (tanto 'pending' como 'approved')
  UPDATE loads
  SET 
    payment_status = 'applied',
    updated_at = now()
...
COMMENT ON FUNCTION public.mark_driver_as_paid_with_validation IS 'Marca payroll como pagado y actualiza payment_status de loads y fuel_expenses a applied (tanto pending como approved)';

-- 3. Actualizar también la función mark_multiple_drivers_as_paid_with_validation
...
COMMENT ON FUNCTION public.mark_multiple_drivers_as_paid_with_validation IS 'Marca múltiples payrolls como pagados y actualiza loads y fuel_expenses (tanto pending como approved)';
