-- Corregir dos problemas críticos:
-- 1. Validar month_week al crear instancias mensuales en generate_recurring_expense_instances_on_template
-- 2. Recalcular payroll después de eliminar instancias en UPDATE
...
BEGIN
  ...
  -- ✅ CORRECCIÓN #2: Si es UPDATE, eliminar instancias y guardar payrolls afectados para recalcular
  IF TG_OP = 'UPDATE' THEN
    -- Guardar lista de payrolls afectados ANTES de eliminar
    SELECT ARRAY_AGG(DISTINCT ei.payment_period_id) INTO v_affected_payrolls
    ...
    -- ✅ CORRECCIÓN #2: Recalcular payrolls afectados después de eliminar
  ...
  WHILE v_period_date <= v_end_date LOOP
    ...
    -- ✅ CORRECCIÓN #1: Para frecuencia mensual, verificar month_week
...
'Genera instancias de gastos recurrentes validando month_week para gastos mensuales y recalcula payrolls al actualizar';