-- Fix load percentage deductions calculation to include loads in date range
-- even if payment_period_id is not yet assigned
...
BEGIN
  ...
  WHERE l.driver_user_id = driver_id
    ...
    AND (
      ...
      OR
      -- Include loads in the date range (for newly created loads without payment_period_id)
