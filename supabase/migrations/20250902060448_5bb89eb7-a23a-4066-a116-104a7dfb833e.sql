-- Fix column reference ambiguity in create_payment_period_if_needed function
...
DECLARE
  ...
  calculated_period_start_date DATE;
  calculated_period_end_date DATE;
  frequency TEXT;
  cycle_start_day INTEGER;
  max_future_date DATE := CURRENT_DATE + INTERVAL '30 days';
  driver_calc_id UUID;
BEGIN
  ...
  IF frequency = 'weekly' THEN
    ...
    calculated_period_start_date := target_date - (EXTRACT(DOW FROM target_date)::INTEGER - 1) * INTERVAL '1 day';
    calculated_period_end_date := calculated_period_start_date + INTERVAL '6 days';
    
    RAISE LOG 'create_payment_period_if_needed: Weekly calculation - start=%, end=%', 
      calculated_period_start_date, calculated_period_end_date;
  ...
  ELSIF frequency = 'biweekly' THEN
    ...
    BEGIN
      ...
      calculated_period_start_date := target_date - (days_since_cycle_start % 14) * INTERVAL '1 day';
      calculated_period_end_date := calculated_period_start_date + INTERVAL '13 days';
  ...
  ELSE -- monthly
    calculated_period_start_date := DATE_TRUNC('month', target_date)::DATE;
    calculated_period_end_date := (DATE_TRUNC('month', target_date) + INTERVAL '1 month - 1 day')::DATE;
    
    RAISE LOG 'create_payment_period_if_needed: Monthly calculation - start=%, end=%', 
      calculated_period_start_date, calculated_period_end_date;
  END IF;

  -- Check if period already exists - FIX: Explicitly qualify all column references
  SELECT cpp.id INTO existing_period_id
  FROM company_payment_periods cpp
  WHERE cpp.company_id = target_company_id
    AND cpp.period_start_date = calculated_period_start_date
    AND cpp.period_end_date = calculated_period_end_date;
  ...
  IF existing_period_id IS NOT NULL THEN
    ...
    IF target_user_id IS NOT NULL THEN
      ...
      WHERE dpc.company_payment_period_id = existing_period_id
        AND dpc.driver_user_id = target_user_id;
  ...
  INSERT INTO company_payment_periods (
    ...
    target_company_id,
    calculated_period_start_date,
    calculated_period_end_date,
    frequency,
    'open'
  ) RETURNING id INTO period_id;

  RAISE LOG 'create_payment_period_if_needed: Created new period % for company %, dates: % - %',
    period_id, target_company_id, calculated_period_start_date, calculated_period_end_date;
