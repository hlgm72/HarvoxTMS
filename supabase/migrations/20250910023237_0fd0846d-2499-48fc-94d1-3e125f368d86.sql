-- Fix driver period recalculation by using existing trigger functions
-- Use the proven auto_recalculate_driver_payment_period_simple function
...
BEGIN
  ...
  -- 🚨 FIX: Use existing proven trigger function for recalculation
  -- Call the same function that the triggers use to ensure consistency
  IF result_load.driver_user_id IS NOT NULL THEN
    BEGIN
      -- Use the existing trigger function that's already proven to work
      PERFORM auto_recalculate_driver_payment_period_simple();
      
      RAISE NOTICE 'SQL: ✅ Driver period recalculation triggered for driver %, operation: %', 
        result_load.driver_user_id, operation_type;
