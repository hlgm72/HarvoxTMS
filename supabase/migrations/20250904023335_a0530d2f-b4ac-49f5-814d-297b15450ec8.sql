-- Fix simple_load_operation_with_deductions function to remove company_id references
CREATE OR REPLACE FUNCTION public.simple_load_operation_with_deductions(operation_type text, load_data jsonb, stops_data jsonb[], load_id_param uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF stops_data IS NOT NULL THEN
    ...
    -- Insert new stops
    FOR stop_data IN SELECT * FROM jsonb_array_elements(stops_data) LOOP
