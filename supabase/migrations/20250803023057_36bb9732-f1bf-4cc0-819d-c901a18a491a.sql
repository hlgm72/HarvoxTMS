-- Primero, verificar si ya existen los tipos de expense
DO $$
BEGIN
  -- Crear tipos de expense para los porcentajes automáticos si no existen
  IF NOT EXISTS (SELECT 1 FROM public.expense_types WHERE name = 'Leasing Fee') THEN
    INSERT INTO public.expense_types (name, description, category, is_active) 
    VALUES ('Leasing Fee', 'Cargo por arrendamiento de equipo basado en porcentaje de carga', 'percentage_deduction', true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_types WHERE name = 'Factoring Fee') THEN
    INSERT INTO public.expense_types (name, description, category, is_active) 
    VALUES ('Factoring Fee', 'Cargo por factoraje basado en porcentaje de carga', 'percentage_deduction', true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.expense_types WHERE name = 'Dispatching Fee') THEN
    INSERT INTO public.expense_types (name, description, category, is_active) 
    VALUES ('Dispatching Fee', 'Cargo por despacho basado en porcentaje de carga', 'percentage_deduction', true);
  END IF;
END $$;
...
BEGIN
  ...
  IF load_record.leasing_percentage > 0 AND leasing_expense_type_id IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      ...
      AND driver_user_id = load_record.driver_user_id
