-- Fix the infinite recursion issue in simple_load_operation
CREATE OR REPLACE FUNCTION public.simple_load_operation(load_data jsonb, stops_data jsonb, operation_mode text DEFAULT 'create')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Get user's company (take the first active one)
  ...
  IF operation_mode = 'edit' THEN
    ...
    -- Simple existence check - just verify the load exists and was created by current user
    IF NOT EXISTS (
      SELECT 1 FROM loads
    ...
    DELETE FROM load_stops WHERE load_id = target_load_id;
  ...
  result_data := jsonb_build_object(
    ...
    'message', CASE 
      WHEN operation_mode = 'edit' THEN 'Load updated successfully'
      ELSE 'Load created successfully'
    END
  );
