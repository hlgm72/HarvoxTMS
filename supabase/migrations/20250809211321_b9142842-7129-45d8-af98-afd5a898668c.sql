-- Corregir la función para permitir que superadmins eliminen usuarios de cualquier empresa
...
DECLARE
  ...
  is_company_owner BOOLEAN := false;
BEGIN
  ...
  -- Si no es superadmin, verificar si es company_owner de alguna empresa donde está el usuario objetivo
  IF NOT is_superadmin THEN
    ...
    -- Si no es company_owner de ninguna empresa del usuario objetivo, denegar acceso
    ...
    END IF;
  ELSE
    -- Si es superadmin, obtener las empresas del usuario objetivo para el log
    SELECT ARRAY(
      SELECT company_id 
      FROM user_company_roles 
      WHERE user_id = user_id_param
  ...
  RETURN jsonb_build_object(
    'success', true,
    'message', CASE 
      WHEN is_superadmin THEN 'Usuario eliminado exitosamente por superadministrador'
      ELSE 'Usuario eliminado exitosamente por propietario de empresa'
    END,
    'user_email', user_email,
    'affected_companies', target_user_companies,
    'deleted_by_superadmin', is_superadmin,
