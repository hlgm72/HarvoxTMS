-- FIX SUPABASE SECURITY WARNINGS (FINAL CORRECTED VERSION)
...
-- Fix expense_types policies (drop all first, then recreate)
DROP POLICY IF EXISTS "expense_types_delete" ON expense_types;
DROP POLICY IF EXISTS "expense_types_update" ON expense_types;
DROP POLICY IF EXISTS "expense_types_delete_authenticated_only" ON expense_types;
DROP POLICY IF EXISTS "expense_types_update_authenticated_only" ON expense_types;

CREATE POLICY "expense_types_delete_secure"
...
CREATE POLICY "expense_types_update_secure"
...
-- Fix profiles policies (drop all first, then recreate)
DROP POLICY IF EXISTS "Company admins can view profiles of their company users" ON profiles;
DROP POLICY IF EXISTS "Users can update their own profile" ON profiles;
DROP POLICY IF EXISTS "Users can view their own profile" ON profiles;
DROP POLICY IF EXISTS "profiles_authenticated_company_admins_view" ON profiles;
DROP POLICY IF EXISTS "profiles_authenticated_users_update_own" ON profiles;
DROP POLICY IF EXISTS "profiles_authenticated_users_view_own" ON profiles;

CREATE POLICY "profiles_secure_company_admin_view"
...
CREATE POLICY "profiles_secure_user_update_own"
...
-- Fix security_audit_log policy (drop first, then recreate)
DROP POLICY IF EXISTS "security_audit_log_superadmin_only" ON security_audit_log;
DROP POLICY IF EXISTS "security_audit_log_authenticated_superadmin_only" ON security_audit_log;

CREATE POLICY "security_audit_log_secure_superadmin"
