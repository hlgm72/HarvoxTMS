-- Fix UPDATE operation to properly handle deductions by removing old ones first
...
DECLARE
  ...
  old_factoring_amount NUMERIC := 0;
  old_dispatching_amount NUMERIC := 0;
  old_leasing_amount NUMERIC := 0;
...
BEGIN
  ...
  -- For UPDATE operations, get the old load record first
  IF operation_type = 'UPDATE' AND load_id_param IS NOT NULL THEN
    SELECT * INTO old_load_record
    FROM loads 
  ...
  -- For UPDATE: Remove old percentage deductions BEFORE updating the load
  IF operation_type = 'UPDATE' AND old_load_record.driver_user_id IS NOT NULL AND old_load_record.payment_period_id IS NOT NULL THEN
    -- Get driver period calculation ID
    SELECT dpc.id INTO driver_calculation_id
    FROM driver_period_calculations dpc
    WHERE dpc.driver_user_id = old_load_record.driver_user_id
    AND dpc.company_payment_period_id = old_load_record.payment_period_id;
    ...
    IF driver_calculation_id IS NOT NULL THEN
      ...
      -- Calculate old deduction amounts that need to be subtracted
      IF old_load_record.factoring_percentage > 0 THEN
        old_factoring_amount := (old_load_record.total_amount * old_load_record.factoring_percentage / 100);
      END IF;
      
      IF old_load_record.dispatching_percentage > 0 THEN
        old_dispatching_amount := (old_load_record.total_amount * old_load_record.dispatching_percentage / 100);
      END IF;
      
      IF old_load_record.leasing_percentage > 0 THEN
        old_leasing_amount := (old_load_record.total_amount * old_load_record.leasing_percentage / 100);
      END IF;

      -- Remove old deductions
      IF old_factoring_amount > 0 AND factoring_expense_type_id IS NOT NULL THEN
        UPDATE expense_instances 
        SET amount = amount - old_factoring_amount
      ...
      IF old_dispatching_amount > 0 AND dispatching_expense_type_id IS NOT NULL THEN
        UPDATE expense_instances 
        SET amount = amount - old_dispatching_amount
        WHERE payment_period_id = driver_calculation_id 
      ...
      IF old_leasing_amount > 0 AND leasing_expense_type_id IS NOT NULL THEN
        UPDATE expense_instances 
        SET amount = amount - old_leasing_amount
        WHERE payment_period_id = driver_calculation_id 
  ...
  IF array_length(stops_data, 1) > 0 THEN
    ...
    FOR stop_data IN SELECT unnest(stops_data) LOOP
      INSERT INTO load_stops (
        ...
        'id', load_stops.id,
        'stop_type', load_stops.stop_type,
        'company_name', load_stops.company_name,
        'city', load_stops.city,
        'state', load_stops.state
      ) INTO stop_result;
  ...
  -- Handle percentage deductions if driver is assigned (now add NEW deductions)
  IF result_load.driver_user_id IS NOT NULL THEN
    ...
    -- Get expense type IDs for percentage deductions (refresh these IDs)
    SELECT id INTO factoring_expense_type_id 
    FROM expense_types 
    WHERE name = 'Factoring Fee' AND category = 'percentage_deduction';
    ...
    -- Calculate NEW amounts using the updated load's total_amount
    ...
    -- Add NEW deductions (these will be added to any remaining deductions)
  ...
  RETURN jsonb_build_object(
    ...
    'deductions_processed', jsonb_build_object(
      'old_deductions_removed', CASE WHEN operation_type = 'UPDATE' THEN 
        jsonb_build_object(
          'factoring_amount', old_factoring_amount,
          'dispatching_amount', old_dispatching_amount,
          'leasing_amount', old_leasing_amount
        ) ELSE null END,
      'new_deductions_added', jsonb_build_object(
