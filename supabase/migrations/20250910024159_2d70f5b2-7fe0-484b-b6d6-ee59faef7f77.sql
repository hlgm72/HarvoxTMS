-- Fix driver period recalculation using correct functions with parameters
-- Use ensure_driver_period_calculation_exists with proper parameters
...
BEGIN
  ...
  IF result_load.driver_user_id IS NOT NULL AND target_payment_period_id IS NOT NULL THEN
    BEGIN
      RAISE NOTICE 'SQL: Starting driver period recalculation - Driver: %, Period: %', 
      ...
      RAISE NOTICE 'SQL: Calculation ID obtained: %', calculation_id;
      ...
      IF calculation_id IS NOT NULL THEN
        ...
        END IF;
      ELSE
        RAISE NOTICE 'SQL: ❌ Failed to get calculation_id for driver period';
      END IF;
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'SQL: ⚠️ Driver recalculation error: %', SQLERRM;
