-- Drop and recreate functions to fix parameter order and SQL ambiguity issues

-- Drop existing functions first
DROP FUNCTION IF EXISTS calculate_driver_payment_period_v2(UUID, UUID);
DROP FUNCTION IF EXISTS recalculate_driver_payment_period(UUID, UUID);
DROP FUNCTION IF EXISTS auto_recalculate_driver_payment_period();

-- 1. Recreate calculate_driver_payment_period_v2 function with fixed column ambiguity
...
BEGIN
  ...
  SELECT COALESCE(SUM(fe.total_amount), 0) INTO total_fuel_amount
...
-- 2. Recreate recalculate_driver_payment_period with proper validation
...
-- 3. Recreate auto_recalculate_driver_payment_period trigger function with correct logic
