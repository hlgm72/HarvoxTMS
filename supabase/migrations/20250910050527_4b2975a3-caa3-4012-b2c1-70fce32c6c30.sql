...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM expense_types WHERE name = 'Dispatching' AND category = 'percentage_deduction') THEN
    INSERT INTO public.expense_types (name, category, description) 
    VALUES ('Dispatching', 'percentage_deduction', 'Percentage-based dispatching fees');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM expense_types WHERE name = 'Factoring' AND category = 'percentage_deduction') THEN
    INSERT INTO public.expense_types (name, category, description) 
    VALUES ('Factoring', 'percentage_deduction', 'Percentage-based factoring fees');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM expense_types WHERE name = 'Leasing' AND category = 'percentage_deduction') THEN
    INSERT INTO public.expense_types (name, category, description) 
    VALUES ('Leasing', 'percentage_deduction', 'Percentage-based leasing fees');
  END IF;
END $$;
