-- Fix percentage deductions recalculation when editing loads (corrected syntax)
...
-- Update the main load operation function with fixed syntax
...
DECLARE
  ...
  driver_period_calculation_id UUID;
...
BEGIN
  ...
  IF operation_type = 'CREATE' OR load_id_param IS NULL THEN
    ...
    -- Get the load number from the created record
    load_number_text := result_load.load_number;
  ELSE
    ...
    -- Get the load number from the updated record
    load_number_text := result_load.load_number;
  END IF;
