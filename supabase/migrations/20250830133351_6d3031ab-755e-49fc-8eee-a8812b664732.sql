-- Fix all ambiguous column references in simple_load_operation function
...
BEGIN
  ...
  -- Check for duplicate load number (only for CREATE or if load_number changed in UPDATE)
  IF operation_type = 'CREATE' THEN
    ...
    END IF;
  ELSIF operation_type = 'UPDATE' AND load_id IS NOT NULL THEN
    IF EXISTS (
      SELECT 1 FROM loads load_table
      JOIN user_company_roles ucr ON load_table.driver_user_id = ucr.user_id
      WHERE ucr.company_id = target_company_id
      AND load_table.load_number = load_number_text
      AND load_table.id != load_id
    ) THEN
      RAISE EXCEPTION 'Error en operación de carga: El número de carga % ya existe', load_number_text;
    END IF;
  END IF;
