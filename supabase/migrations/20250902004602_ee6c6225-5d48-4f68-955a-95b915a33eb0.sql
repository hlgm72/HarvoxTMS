...
-- Resolver el error: column reference "is_locked" is ambiguous

-- Corregir la función is_driver_paid_in_period para especificar la tabla
DROP FUNCTION IF EXISTS public.is_driver_paid_in_period(UUID, UUID);
...
AS $$
  SELECT COALESCE(
    (SELECT dpc.payment_status = 'paid'
...
-- Corregir la política DELETE de expense_instances para evitar ambigüedad
...
-- Corregir la función de limpieza para ser más específica con las tablas
...
-- Mejorar la función cleanup_empty_payment_period con alias de tabla
