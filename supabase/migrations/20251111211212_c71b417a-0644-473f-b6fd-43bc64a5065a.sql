-- Corregir simple_load_operation_with_deductions para CREAR períodos automáticamente
-- Problema: Solo busca períodos existentes, no los crea si no existen
-- Resultado: Cargas quedan sin payment_period_id
...
BEGIN
  ...
  -- ========================================
  -- 3. ASIGNAR PAYMENT PERIOD (CORREGIDO: AHORA CREA EL PERÍODO)
  -- ========================================
  IF v_driver_user_id IS NOT NULL THEN
    ...
    IF v_target_date IS NOT NULL THEN
      -- ✅ CORRECCIÓN: Crear o buscar período usando la función centralizada
      ...
      -- Asignar período a la carga
      IF v_payment_period_id IS NOT NULL THEN
        UPDATE loads
        SET payment_period_id = v_payment_period_id
        WHERE id = v_load_id;
        
        RAISE LOG '✅ Payment period % assigned to load % using % (date: %)', 
          v_payment_period_id, v_load_id, v_load_assignment_criteria, v_target_date;
      ELSE
        RAISE LOG '⚠️ Could not create payment period for date % (criteria: %)', 
...
'Función ACID para crear/actualizar cargas. Respeta load_assignment_criteria (pickup_date o delivery_date) y CREA períodos automáticamente con ISO 8601';