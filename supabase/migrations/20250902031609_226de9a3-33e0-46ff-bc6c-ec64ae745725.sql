-- Fix percentage deductions update logic for load editing
-- When editing a load, replace existing percentage deductions instead of adding to them
...
BEGIN
  ...
  IF result_load.driver_user_id IS NOT NULL THEN
    ...
    IF driver_period_calc_id IS NOT NULL THEN
      -- For EDIT operations, delete existing percentage deductions first to avoid accumulation
      IF operation_type = 'UPDATE' THEN
        DELETE FROM expense_instances 
        WHERE payment_period_id = driver_period_calc_id 
        AND user_id = result_load.driver_user_id 
        AND expense_type_id IN (factoring_expense_type_id, dispatching_expense_type_id, leasing_expense_type_id);
      END IF;
