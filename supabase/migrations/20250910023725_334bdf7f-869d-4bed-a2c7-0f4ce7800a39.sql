-- Fix load status logic to automatically set "assigned" when driver is assigned
-- Update the load operation function to handle status correctly
...
BEGIN
  ...
  -- 🚨 CRITICAL FIX: Calculate status based on driver assignment
  -- If status is explicitly provided, use it; otherwise auto-calculate
  ...
  IF operation_type = 'CREATE' THEN
    INSERT INTO loads (
      ...
      calculated_status,  -- Use calculated status
  ...
  -- Use existing proven trigger function for recalculation
  IF result_load.driver_user_id IS NOT NULL THEN
    BEGIN
      PERFORM auto_recalculate_driver_payment_period_simple();
      
      RAISE NOTICE 'SQL: ✅ Driver period recalculation triggered for driver %, operation: %, status: %', 
        result_load.driver_user_id, operation_type, result_load.status;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'SQL: ⚠️ Driver recalculation warning: %', SQLERRM;
    END;
  END IF;
