...
DECLARE
  ...
  total_other_income_calc NUMERIC := 0;
...
BEGIN
  ...
  -- Calcular total de otros ingresos (other_income_instances)
  SELECT COALESCE(SUM(amount), 0) INTO total_other_income_calc
  FROM other_income_instances
  WHERE payment_period_id = period_calc_id
  AND status IN ('applied', 'planned')
  AND amount > 0;
  
  -- Calcular total de gastos de combustible
  ...
  SET total_income = gross_earnings + total_other_income_calc
  WHERE id = period_calc_id;
  
  -- Calcular pago neto
  net_payment_calc := period_record.gross_earnings + total_other_income_calc - total_deductions_calc - total_fuel_expenses_calc;
  
  -- Actualizar totales en driver_period_calculations
  UPDATE driver_period_calculations
  SET 
    total_deductions = total_deductions_calc,
    other_income = total_other_income_calc,
    fuel_expenses = total_fuel_expenses_calc,
    total_income = period_record.gross_earnings + total_other_income_calc,
...
-- Crear triggers para other_income_instances (si existe la tabla)
DROP TRIGGER IF EXISTS other_income_instances_recalculate_totals ON other_income_instances;
CREATE TRIGGER other_income_instances_recalculate_totals
  AFTER INSERT OR UPDATE OR DELETE ON other_income_instances
  FOR EACH ROW
  EXECUTE FUNCTION trigger_recalculate_period_totals();