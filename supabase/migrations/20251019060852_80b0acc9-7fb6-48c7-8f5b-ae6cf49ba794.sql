-- Function to recalculate driver period calculations when fuel expenses change
...
-- Create trigger for UPDATE operations on fuel_expenses
DROP TRIGGER IF EXISTS trigger_recalculate_on_fuel_update ON fuel_expenses;
CREATE TRIGGER trigger_recalculate_on_fuel_update
AFTER UPDATE ON fuel_expenses
FOR EACH ROW
EXECUTE FUNCTION recalculate_driver_period_on_fuel_change();

-- Create trigger for DELETE operations on fuel_expenses
DROP TRIGGER IF EXISTS trigger_recalculate_on_fuel_delete ON fuel_expenses;
CREATE TRIGGER trigger_recalculate_on_fuel_delete
