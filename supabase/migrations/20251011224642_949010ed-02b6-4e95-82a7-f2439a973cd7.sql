-- Fix simple_load_operation_with_deductions to not use company_id column in loads table
...
BEGIN
  ...
  -- Create or update the load (WITHOUT company_id column)
  IF v_is_update THEN
  ...
  ELSE
    -- Create new load (WITHOUT company_id column)
    INSERT INTO loads (
      load_number,
      po_number,
      driver_user_id,
