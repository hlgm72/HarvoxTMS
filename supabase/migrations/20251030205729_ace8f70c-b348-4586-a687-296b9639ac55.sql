...
-- Mejorar simple_load_operation_with_deductions para limpiar períodos vacíos al editar
-- Cuando se mueve una carga de un período a otro, el período viejo puede quedar vacío
...
BEGIN
  ...
  -- Obtener company_id desde el usuario autenticado
  ...
  ELSE
    ...
    INSERT INTO loads (
      ...
      notes, customer_name, company_id, pickup_date, delivery_date,
  ...
  LOOP
    INSERT INTO load_stops (
      ...
      NULLIF(v_stop->>'scheduled_time', ''),
      NULLIF(v_stop->>'actual_date', '')::DATE,
      NULLIF(v_stop->>'actual_time', ''),
  ...
  -- ========================================
  -- 3. ASIGNAR PERÍODO DE PAGO (SOLO SI HAY CONDUCTOR)
  -- ========================================
  IF v_driver_user_id IS NOT NULL THEN
    -- Usar pickup_date como referencia principal para el período
    SELECT create_payment_period_if_needed(
      v_company_id,
      v_driver_user_id,
      COALESCE(v_pickup_date, v_delivery_date, CURRENT_DATE)
    ) INTO v_payment_period_id;

    -- Actualizar el load con el período correcto
    UPDATE loads 
    SET payment_period_id = v_payment_period_id
    WHERE id = v_load_id;

    -- ========================================
    -- 4. GENERAR DEDUCCIONES AUTOMÁTICAS
    -- ========================================
    SELECT generate_load_automatic_deductions(v_load_id, v_payment_period_id)
    INTO v_deduction_result;
  END IF;

  -- ========================================
  -- 5. LIMPIAR PERÍODO VIEJO SI QUEDÓ VACÍO (SOLO EN EDICIÓN)
  -- ========================================
  IF v_is_update AND v_old_payment_period_id IS NOT NULL AND v_old_payment_period_id != v_payment_period_id THEN
    -- Verificar si el período viejo tiene algún dato
    SELECT EXISTS (
      SELECT 1 FROM loads WHERE payment_period_id = v_old_payment_period_id
      UNION ALL
      SELECT 1 FROM fuel_expenses WHERE payment_period_id = v_old_payment_period_id
      UNION ALL
      SELECT 1 FROM expense_instances WHERE payment_period_id = v_old_payment_period_id
      UNION ALL
      SELECT 1 FROM other_income WHERE payment_period_id = v_old_payment_period_id
    ) INTO period_has_any_data;

    -- Si el período quedó completamente vacío, eliminarlo
    IF NOT period_has_any_data THEN
      DELETE FROM company_payment_periods WHERE id = v_old_payment_period_id;
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    'success', TRUE,
    'load', jsonb_build_object('id', v_load_id),
    'payment_period_id', v_payment_period_id,
    'automatic_deductions', v_deduction_result
  );
