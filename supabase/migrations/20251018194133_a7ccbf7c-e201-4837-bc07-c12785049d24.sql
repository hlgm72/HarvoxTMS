...
-- ====================================================================
-- 🚨 SOLUCIÓN DEFINITIVA: SIMPLIFICAR recalculación de períodos
-- ====================================================================
-- 
-- PROBLEMA RAÍZ IDENTIFICADO:
-- Cuando se edita un load, se intenta recalcular períodos antiguos que 
-- podrían haber sido eliminados, causando errores de FK que abortan 
-- toda la transacción.
--
-- SOLUCIÓN:
-- SOLO recalcular el período ACTUAL del load. Los períodos anteriores
-- quedan como están. Esto elimina el problema de raíz.
-- ====================================================================
...
BEGIN
  ...
  IF v_relevant_date IS NOT NULL THEN
    ...
    IF v_driver_user_id IS NOT NULL THEN
      ...
      -- 🔥 SOLUCIÓN SIMPLIFICADA: Solo recalcular el período ACTUAL
      -- No intentamos recalcular períodos antiguos que podrían haber sido eliminados
      BEGIN
        PERFORM recalculate_period_percentage_deductions(v_company_payment_period_id, v_driver_user_id);
        RAISE LOG 'simple_load_operation: Recalculated current period % for driver %', v_company_payment_period_id, v_driver_user_id;
      EXCEPTION 
...
COMMENT ON FUNCTION simple_load_operation_with_deductions IS 'Simplified version that only recalculates the current period, avoiding FK errors from deleted old periods';
