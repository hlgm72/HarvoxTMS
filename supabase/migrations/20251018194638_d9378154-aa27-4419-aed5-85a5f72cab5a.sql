...
-- ====================================================================
-- 🚨 SOLUCIÓN TEMPORAL MIENTRAS SE INVESTIGA EL PROBLEMA
-- ====================================================================
-- 
-- PROBLEMA: Los recálculos automáticos de deducciones están causando
-- errores de FK que no podemos resolver sin más investigación.
--
-- SOLUCIÓN TEMPORAL: Deshabilitar completamente el recálculo automático
-- durante la edición de loads. El load se guardará correctamente y
-- las deducciones se pueden recalcular manualmente después.
-- ====================================================================
...
BEGIN
  ...
  IF v_relevant_date IS NOT NULL THEN
    ...
    IF v_driver_user_id IS NOT NULL THEN
      ...
      -- 🔥 TEMPORALMENTE DESHABILITADO: Recálculo automático de deducciones
      -- Esto está causando errores de FK que necesitan más investigación
      -- Las deducciones se pueden recalcular manualmente después
      RAISE LOG 'simple_load_operation: Skipping automatic percentage deduction recalculation (temporarily disabled)';
      
      -- Calcular totales del payroll sin recalcular deducciones primero
      BEGIN
...
COMMENT ON FUNCTION simple_load_operation_with_deductions IS 'TEMPORARY: Percentage deduction recalculation disabled while investigating FK errors. Load saves work, deductions can be recalculated manually.';
