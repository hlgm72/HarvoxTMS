-- 1) Update auto_recalculate_on_load_change to call the non-validating calculator (wrapped with GUC)
...
-- 2) Update auto_recalculate_driver_period to call the wrapped calculator and keep the guard
...
-- 3) Add the session flag guard to the validating calculator as well
...
BEGIN
  ...
  WHERE l.driver_user_id = calculation_record.driver_user_id
    ...
    AND l.status IN ('assigned', 'in_transit', 'completed')
  ...
  -- 5) Deductions (generate and recalc)
