-- MEGA SECURITY FIX: Secure all publicly accessible tables and views
-- This addresses ALL security vulnerabilities found in the scan

-- ===== 1. SECURE COMPANIES_PUBLIC VIEW =====
-- Revoke public access and enable proper permissions
...
-- Create RLS policies for load-related tables if they exist

-- Check and secure load_details_with_dates (if it's a view)
DO $$ 
BEGIN
...
-- Check and secure loads_complete (if it's a view)
DO $$ 
BEGIN
...
-- Check and secure equipment_status_summary (if it's a view)
DO $$ 
BEGIN
...
-- These should only be accessible to superadmins
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'system_health_log' AND table_schema = 'public') THEN
    ...
    -- Only superadmins can access system health logs
    EXECUTE 'CREATE POLICY IF NOT EXISTS "system_health_log_superadmin_only" ON public.system_health_log FOR ALL USING (
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'system_backups' AND table_schema = 'public') THEN
    ...
    -- Only superadmins can access backup information
    EXECUTE 'CREATE POLICY IF NOT EXISTS "system_backups_superadmin_only" ON public.system_backups FOR ALL USING (
      (SELECT auth.role()) = ''authenticated'' AND 
      (SELECT auth.uid()) IS NOT NULL AND 
      COALESCE(((SELECT auth.jwt()) ->> ''is_anonymous'')::boolean, false) = false AND 
      EXISTS (
...
-- deployment_log is already secured based on previous migrations
...
INSERT INTO public.company_data_access_log (company_id, accessed_by, access_type, action)
SELECT 
  c.id,
  auth.uid(),
  'security_hardening',
  'Applied comprehensive security policies to all public tables and views'
