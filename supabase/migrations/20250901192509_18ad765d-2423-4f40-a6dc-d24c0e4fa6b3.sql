-- 🐛 FIX CRÍTICO: simple_load_operation no procesa internal_dispatcher_user_id correctamente
-- El problema está en la lógica CASE WHEN que evalúa como NULL
...
DECLARE
  ...
  dispatcher_id_value UUID; -- ✅ Nueva variable para debug
BEGIN
  ...
  -- ✅ NUEVO: Procesar dispatcher_id correctamente
  -- Extraer el valor y verificar si es válido
  IF load_data->>'internal_dispatcher_user_id' IS NOT NULL AND 
     load_data->>'internal_dispatcher_user_id' != '' THEN
    dispatcher_id_value := (load_data->>'internal_dispatcher_user_id')::UUID;
  ELSE
    dispatcher_id_value := NULL;
  END IF;

  -- ✅ DEBUG LOGS
  RAISE LOG '🔍 DISPATCHER DEBUG: raw value = "%"', load_data->>'internal_dispatcher_user_id';
