-- Fix the trigger function to correctly handle other_income payment_period_id
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'other_income' THEN
    driver_id := COALESCE(NEW.driver_user_id, OLD.driver_user_id);
    -- For other_income, payment_period_id is directly a company_payment_periods.id
  ...
  IF company_period_id IS NULL THEN
    ...
    ELSE
      -- For loads and fuel_expenses, payment_period_id is company_payment_periods.id
