-- ============================================================================
-- MIGRACIÓN: Corregir funciones de combustible eliminando columnas inexistentes
-- ============================================================================

-- Primero eliminar la función delete para poder cambiarla
DROP FUNCTION IF EXISTS public.delete_fuel_expense_with_validation(uuid);

-- ============================================================================
-- 1. Recrear función principal de creación/actualización
-- ============================================================================
...
BEGIN
  ...
  IF v_is_update THEN
    ...
    -- Recalcular conductor anterior si cambió
    IF v_old_driver_id IS NOT NULL AND v_old_period_id IS NOT NULL THEN
      ...
      JOIN user_payrolls up ON up.id = v_old_period_id
  ...
  -- Recalcular conductor actual
  IF v_driver_user_id IS NOT NULL AND v_payment_period_id IS NOT NULL THEN
    SELECT company_payment_period_id INTO v_company_payment_period_id
    FROM company_payment_periods cpp
    JOIN user_payrolls up ON up.id = v_payment_period_id
    WHERE up.company_payment_period_id = cpp.id
    LIMIT 1;
...
BEGIN
  SELECT driver_user_id, payment_period_id
  INTO v_driver_user_id, v_payment_period_id
  FROM fuel_expenses
  WHERE id = expense_id;
