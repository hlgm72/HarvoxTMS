...
-- 🔧 CORRECCIÓN CRÍTICA: gross_earnings debe ser BRUTO, sin deducciones
-- BUG: La función estaba restando los porcentajes del gross_earnings
-- SOLUCIÓN: gross_earnings = suma total de loads, deducciones van a total_deductions
...
BEGIN
    ...
    -- 🔧 CORRECCIÓN: Calculate gross earnings BRUTO (sin deducciones)
    -- Antes estaba restando porcentajes, ahora es la suma pura
    ...
    -- Calculate load-related deductions (estos SI son deducciones)
    SELECT COALESCE(SUM(
        l.total_amount * (
    ...
    v_result := json_build_object(
        ...
        'other_income', v_other_income,
        'fuel_expenses', v_fuel_expenses,
        'load_deductions', v_load_deductions,
        'total_deductions', v_total_deductions,
        'net_payment', v_net_payment
    );
