-- Fix the CTE query in the deductions function to properly include all loads
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(l.total_amount * COALESCE(l.leasing_percentage, oo.leasing_percentage, c.default_leasing_percentage, 0) / 100), 0),
    COALESCE(SUM(l.total_amount * COALESCE(l.factoring_percentage, oo.factoring_percentage, c.default_factoring_percentage, 0) / 100), 0),
    COALESCE(SUM(l.total_amount * COALESCE(l.dispatching_percentage, oo.dispatching_percentage, c.default_dispatching_percentage, 0) / 100), 0)
