-- Corregir la función de recálculo para que busque cargas basándose en company_payment_period + driver_user_id
-- El problema es que las cargas apuntan al company_payment_period, no al driver_period_calculation
...
BEGIN
  RAISE NOTICE '🔄 v4.0-FIXED-LOAD-LOOKUP: Iniciando recálculo para conductor % en período %', target_driver_user_id, target_period_id;
  ...
  -- ============================================================================
  -- PASO 2: BUSCAR CARGAS BASÁNDOSE EN COMPANY_PAYMENT_PERIOD + DRIVER_USER_ID
  -- ============================================================================
  
  -- CAMBIO CRÍTICO: Buscar cargas que apunten al company_payment_period Y sean del conductor
  SELECT 
  ...
  FROM loads l
  WHERE l.payment_period_id = company_period_id  -- Cargas apuntan al company_payment_period
    AND l.driver_user_id = target_driver_user_id  -- Del conductor específico
  ...
  RAISE NOTICE '📊 v4.0 Cargas encontradas: % cargas, Bruto: $%, Dispatching: $%, Factoring: $%, Leasing: $%', 
    aggregated_totals.load_count, aggregated_totals.gross_earnings, total_dispatching_fees, total_factoring_fees, total_leasing_fees;
  ...
  IF total_dispatching_fees > 0 THEN
    INSERT INTO expense_instances (
      payment_period_id, user_id, expense_type_id, amount, description, status, applied_at, applied_by, created_by
    ) VALUES (
      driver_calc_id, target_driver_user_id, dispatching_expense_type_id, total_dispatching_fees,
      'Dispatching fees calculated from loads', 'applied', now(), NULL, NULL
    );
  ...
  IF total_factoring_fees > 0 THEN
    INSERT INTO expense_instances (
      ...
      driver_calc_id, target_driver_user_id, factoring_expense_type_id, total_factoring_fees,
      'Factoring fees calculated from loads', 'applied', now(), NULL, NULL
    );
  ...
  IF total_leasing_fees > 0 THEN
    INSERT INTO expense_instances (
      ...
      driver_calc_id, target_driver_user_id, leasing_expense_type_id, total_leasing_fees,
      'Leasing fees calculated from loads', 'applied', now(), NULL, NULL
    );
  ...
  SET
    ...
    total_income = aggregated_totals.gross_earnings + 0,
  ...
  RAISE NOTICE '✅ v4.0-FIXED-LOAD-LOOKUP: Recálculo completado correctamente para conductor %', target_driver_user_id;
...
EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'ERROR_OPERATION_FAILED: Error en recálculo v4.0-FIXED-LOAD-LOOKUP: %', SQLERRM;
END;
$function$;

-- Ahora ejecutar el recálculo con la función corregida
