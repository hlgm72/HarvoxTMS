-- Fix field name: use payment_period_id instead of driver_calculation_id
...
BEGIN
  ...
  IF user_payroll_id IS NOT NULL THEN
    SELECT payment_period_id INTO company_period_id
    FROM user_payrolls
  ...
  IF user_payroll_id IS NOT NULL THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE driver_calculation_id = user_payroll_id
    ) AND NOT EXISTS (
