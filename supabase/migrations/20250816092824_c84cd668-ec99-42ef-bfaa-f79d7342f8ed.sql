-- Final RLS performance optimization - fix companies table auth calls and remove duplicate policies
...
CREATE POLICY "loads_archive_unified" ON public.loads_archive
FOR ALL USING (
  (SELECT auth.uid()) IS NOT NULL AND
  COALESCE(((SELECT auth.jwt())->>'is_anonymous')::boolean, false) = false AND
  is_user_superadmin_safe((SELECT auth.uid()))
...
-- Keep only the optimized profiles policies (they should already exist)
...
-- Keep only the optimized security_audit_log policy