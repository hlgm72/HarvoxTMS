-- Drop the problematic function and create a simpler one that completely bypasses RLS
DROP FUNCTION IF EXISTS public.simple_load_operation(jsonb, jsonb, text);

-- Create a simpler function that handles load operations without complex RLS interactions
...
BEGIN
  ...
  IF operation_mode = 'edit' THEN
    ...
    DELETE FROM public.load_stops WHERE load_id = load_id;
