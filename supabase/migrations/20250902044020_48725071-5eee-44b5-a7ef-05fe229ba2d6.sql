-- Fix recurring deduction date logic in create_payment_period_if_needed
-- The function was incorrectly applying deductions that start after the period ends
...
BEGIN
  ...
  IF period_id IS NULL THEN
    ...
    LOOP
      ...
      -- FIXED: Generate recurring expense instances with correct date validation
      FOR recurring_expense IN
        SELECT ert.expense_type_id, ert.amount, et.description, et.name, ert.start_date, ert.end_date
        ...
        WHERE ert.user_id = driver_record.user_id
          ...
          -- FIXED: Deduction must start before or during the period
          AND ert.start_date <= period_end
          -- FIXED: If there's an end date, it must be after or during the period
          AND (ert.end_date IS NULL OR ert.end_date >= period_start)
      LOOP
        ...
        IF existing_instance_count = 0 THEN
          INSERT INTO expense_instances (
            ...
            now(),
            current_user_id,
            (SELECT id FROM expense_recurring_templates WHERE expense_type_id = recurring_expense.expense_type_id AND user_id = driver_record.user_id LIMIT 1)
          );

          RAISE LOG 'create_payment_period_if_needed: Created recurring expense % for driver % (period: % to %)', 
            recurring_expense.name, driver_record.user_id, period_start, period_end;
        ELSE
