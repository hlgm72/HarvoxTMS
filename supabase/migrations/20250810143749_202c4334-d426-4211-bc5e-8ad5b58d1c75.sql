-- Revertir el cambio y mantener la función original que solo permite a company_owners eliminar usuarios de sus empresas
CREATE OR REPLACE FUNCTION public.permanently_delete_user_with_validation(
  user_id_param UUID,
...
BEGIN
  ...
  -- Solo los company_owners pueden eliminar usuarios de sus empresas
  IF NOT is_company_owner THEN
    RAISE EXCEPTION 'Solo los propietarios de empresa pueden eliminar usuarios permanentemente de sus empresas';
  END IF;
  ...
  RETURN jsonb_build_object(
    'success', true,
    'message', 'Usuario eliminado exitosamente por propietario de empresa',
