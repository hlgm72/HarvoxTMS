...
CREATE TABLE IF NOT EXISTS public.recurring_expense_exclusions (
  ...
  recurring_template_id UUID NOT NULL REFERENCES recurring_expense_templates(id) ON DELETE CASCADE,
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'excluded_template', template_record.template_name,
...
BEGIN
  ...
  INSERT INTO expense_instances (
    ...
    template_record.template_name || ' (Restaurada)',
    CURRENT_DATE,
    'applied',
    current_user_id,
    now(),
  ...
  RETURN jsonb_build_object(
    ...
    'restored_template', template_record.template_name,
