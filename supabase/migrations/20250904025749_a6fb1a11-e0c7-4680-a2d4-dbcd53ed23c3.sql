-- Fix variable name conflict in calculate_driver_payment_period function
...
DECLARE
    ...
    calc_net_payment numeric := 0;  -- FIXED: renamed to avoid conflict
...
BEGIN
    ...
    calc_net_payment := total_gross_earnings + total_other_income - total_fuel_expenses - total_deductions_amount;
    has_negative := calc_net_payment < 0;
    
    -- PASO 8: Generar mensaje de alerta si hay balance negativo - FIXED format string
    IF has_negative THEN
        alert_msg := 'Balance negativo: El conductor debe $' || ABS(calc_net_payment)::text;
    END IF;
    ...
    SET 
        ...
        net_payment = calc_net_payment,  -- FIXED: use renamed variable
