...
-- Crear triggers para eliminación automática del storage
...
CREATE TRIGGER delete_company_document_storage_trigger
  AFTER DELETE ON company_documents
  FOR EACH ROW
  EXECUTE FUNCTION delete_company_document_storage_file();
...
BEGIN
  ...
  -- Get document information and validate access
  SELECT ld.*, l.company_id INTO doc_record
...
BEGIN
  ...
  -- Validate load exists and user has access
  SELECT l.*, c.id as company_id INTO load_record
  FROM loads l
