-- Corregir función recalculate_payment_period_totals - usar transaction_date para fuel_expenses
...
BEGIN
  ...
  -- Calculate fuel expenses total using transaction_date within period range
  SELECT COALESCE(SUM(total_amount), 0) INTO fuel_total
  FROM fuel_expenses fe
  WHERE fe.driver_user_id = calculation_record.driver_user_id
  AND fe.transaction_date >= calculation_record.period_start_date
  AND fe.transaction_date <= calculation_record.period_end_date;
