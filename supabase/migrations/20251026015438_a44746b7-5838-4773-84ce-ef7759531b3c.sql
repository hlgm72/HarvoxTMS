-- Fix load_stops INSERT statement - remove obsolete columns that moved to facilities table
-- Columns like company_name, address, city, state, zip_code, reference_number, contact_name, contact_phone
-- no longer exist in load_stops. They are now referenced via facility_id.

DROP FUNCTION IF EXISTS simple_load_operation_with_deductions(jsonb, jsonb, uuid) CASCADE;
...
BEGIN
  ...
  -- ✅ FIXED: Only insert valid columns that exist in load_stops table
  -- Removed: company_name, address, city, state, zip_code, reference_number, contact_name, contact_phone
  -- These are now in facilities table, referenced via facility_id
  FOR v_stop IN SELECT * FROM jsonb_array_elements(stops_data)
  LOOP
    INSERT INTO load_stops (
      load_id, stop_number, stop_type, facility_id, special_instructions, driver_notes,
      scheduled_date, scheduled_time, actual_date, actual_time,
      pickup_timezone, delivery_timezone
    ) VALUES (
      v_load_id, 
      (v_stop.value->>'stop_number')::integer, 
      v_stop.value->>'stop_type',
      (v_stop.value->>'facility_id')::uuid,
      NULLIF(v_stop.value->>'special_instructions', ''),
      NULLIF(v_stop.value->>'driver_notes', ''),
      (v_stop.value->>'scheduled_date')::date, 
      (v_stop.value->>'scheduled_time')::time,
      (v_stop.value->>'actual_date')::date,
      (v_stop.value->>'actual_time')::time,
      COALESCE(v_stop.value->>'pickup_timezone', 'America/New_York'),
      COALESCE(v_stop.value->>'delivery_timezone', 'America/New_York')
    );
  END LOOP;
    ...
    IF v_driver_user_id IS NOT NULL THEN
      ...
      IF v_user_payroll_id IS NULL THEN
        INSERT INTO user_payrolls (
          ...
          total_deductions, other_income, net_payment, payment_status, calculated_by,
          payroll_role
        ) VALUES (
          v_driver_user_id, v_company_payment_period_id, v_company_id,
          0, 0, 0, 0, 0, 'calculated', auth.uid(),
          'company_driver'
      ...
      IF v_is_update THEN
        IF v_old_period_id IS NOT NULL AND v_old_period_id != v_company_payment_period_id AND v_old_driver_id IS NOT NULL THEN
          PERFORM recalculate_period_percentage_deductions(v_old_period_id, v_old_driver_id);
        END IF;
        
        IF v_old_driver_id IS NOT NULL AND v_old_driver_id != v_driver_user_id THEN
