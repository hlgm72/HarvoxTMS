-- Fix the ambiguous column reference in calculate_driver_payment_period function
...
DECLARE
    ...
    calculated_net_payment numeric := 0;  -- FIXED: renamed to avoid ambiguity
...
BEGIN
    ...
    -- PASO 4: Calcular gastos de combustible - FIXED: use company_payment_period_id instead of period_calculation_id
    ...
    -- PASO 6: Calcular otros ingresos
    ...
    calculated_net_payment := total_gross_earnings + total_other_income - total_fuel_expenses - total_deductions_amount;
    has_negative := calculated_net_payment < 0;
    
    -- PASO 8: Generar mensaje de alerta si hay balance negativo
    IF has_negative THEN
        alert_msg := format('Balance negativo: El conductor debe $%.2f', ABS(calculated_net_payment));
    END IF;
    
    -- PASO 9: Actualizar el registro de cálculo - FIXED: use explicit variable names
    ...
    SET 
        ...
        net_payment = calculated_net_payment,  -- FIXED: use explicit variable name
