-- Fix the ambiguous column reference in simple_load_operation_with_deductions function
-- The issue is with ON CONFLICT clauses that reference (payment_period_id, expense_type_id)
-- but the actual constraint should be (payment_period_id, expense_type_id, user_id)
