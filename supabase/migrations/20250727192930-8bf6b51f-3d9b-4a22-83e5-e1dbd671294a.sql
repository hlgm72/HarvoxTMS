-- Fix infinite recursion by updating ALL policies that use require_authenticated_user()
-- Must be done before dropping the function

-- First, update all the remaining policies that still use require_authenticated_user()

-- Company payment periods policies
...
TO authenticated
USING (
  (auth.role() = 'authenticated') AND is_company_owner_in_company(company_id)
);
