-- Fix the load status filter to include ALL relevant statuses
...
BEGIN
  RAISE NOTICE '🔄 v4.8-ALL-LOAD-STATUS: Iniciando recálculo para conductor % en período %', target_driver_user_id, target_period_id;
  ...
  -- 2. Calculate gross earnings from loads 🚨 FIX: Include ALL relevant load statuses
  ...
  WHERE l.driver_user_id = target_driver_user_id
    ...
    AND l.status IN ('delivered', 'confirmed', 'assigned', 'completed', 'in_transit', 'created'); -- 🚨 FIX: Include more statuses
  ...
  WHERE l.driver_user_id = target_driver_user_id
    AND l.pickup_date >= calculation_record.period_start_date
    AND l.pickup_date <= calculation_record.period_end_date
    AND l.status IN ('delivered', 'confirmed', 'assigned', 'completed', 'in_transit', 'created'); -- 🚨 FIX: Same statuses
  ...
  SET 
    gross_earnings = calculated_gross_earnings,      -- 🚨 NOW SHOULD INCLUDE ALL LOADS
  ...
  RAISE NOTICE '🎉 FINAL: gross=% (TODAS LAS CARGAS), fuel=%, deductions=%, other=%, net=%', 
