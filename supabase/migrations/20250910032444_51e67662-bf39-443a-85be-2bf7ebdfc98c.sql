-- Fix ambiguous column reference in simple_load_operation_with_deductions
-- Rename load_id parameter to load_id_param to avoid conflicts with table columns
...
BEGIN
  ...
  IF array_length(stops_array, 1) > 0 THEN
    ...
    DELETE FROM load_stops WHERE load_stops.load_id = result_load.id;
