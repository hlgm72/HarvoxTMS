-- Cuarta ronda: Continuar resolviendo warnings de acceso anónimo
...
-- 6. PAYMENT_REPORTS - Reportes de pago
...
FOR ALL
TO service_role
USING (require_authenticated_user() AND (payment_period_id IN ( SELECT pp.id
   FROM payment_periods pp
  WHERE ((pp.driver_user_id = ( SELECT auth.uid() AS uid)) OR (pp.driver_user_id IN ( SELECT ucr.user_id
           FROM user_company_roles ucr
          WHERE ((ucr.company_id IN ( SELECT ucr2.company_id
                   FROM user_company_roles ucr2
                  WHERE ((ucr2.user_id = ( SELECT auth.uid() AS uid)) AND (ucr2.is_active = true)))) AND (ucr.is_active = true))))))))
WITH CHECK (require_authenticated_user() AND (payment_period_id IN ( SELECT pp.id
   FROM payment_periods pp
  WHERE ((pp.driver_user_id = ( SELECT auth.uid() AS uid)) OR (pp.driver_user_id IN ( SELECT ucr.user_id
           FROM user_company_roles ucr
          WHERE ((ucr.company_id IN ( SELECT ucr2.company_id
