-- Corregir la función mark_driver_as_paid_with_validation
-- El problema: usa 'driver_user_id' para other_income cuando la columna se llama 'user_id'
...
BEGIN
  -- Obtener user_id y period_id del payroll
  ...
  -- ORDEN CRÍTICO: Actualizar TODAS las entidades ANTES de marcar payroll como paid
  ...
  -- 3. Marcar other_income como 'applied' (CORREGIDO: usar 'user_id' en vez de 'driver_user_id')
  UPDATE other_income
  SET 
    status = 'applied',
    updated_at = now()
  ...
  -- 4. Marcar expense_instances (deductions) como 'applied'
  ...
  -- 5. AHORA SÍ: Marcar el payroll como 'paid'
...
COMMENT ON FUNCTION public.mark_driver_as_paid_with_validation IS 
'Marca un payroll como pagado. ORDEN CRÍTICO: actualiza loads, fuel, other_income y expense_instances ANTES de marcar payroll como paid.';