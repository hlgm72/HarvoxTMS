-- Fix the UPDATE permission check in simple_load_operation function
...
BEGIN
  ...
  ELSIF operation_type = 'UPDATE' AND load_id_param IS NOT NULL THEN
    -- Simplified verification - just check if load exists and user has admin rights in the company
    IF NOT EXISTS (
      SELECT 1 FROM loads load_table
