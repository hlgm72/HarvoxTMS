-- Fix duplicate expense instance creation in simple_load_operation_with_deductions
-- Use ON CONFLICT DO UPDATE to accumulate expense instance amounts instead of failing
...
BEGIN
  ...
  IF result_load.driver_user_id IS NOT NULL AND payment_period_id_result IS NOT NULL THEN
    ...
    IF driver_calculation_id IS NOT NULL THEN
      ...
      -- FIXED: Use ON CONFLICT DO UPDATE to accumulate expense amounts instead of failing
      IF factoring_amount > 0 AND factoring_expense_type_id IS NOT NULL THEN
        ...
        DO UPDATE SET 
          ...
          description = expense_instances.description || ', Load ' || result_load.load_number,
          updated_at = now();
      END IF;
