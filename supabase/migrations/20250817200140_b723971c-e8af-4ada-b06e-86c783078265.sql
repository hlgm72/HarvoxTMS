...
SELECT proname, prosrc FROM pg_proc WHERE proname LIKE '%create_or_update_load_document%';

-- Drop all versions and recreate properly
DROP FUNCTION IF EXISTS create_or_update_load_document_with_validation(jsonb, uuid);
DROP FUNCTION IF EXISTS create_or_update_load_document_with_validation(jsonb);

-- Create the fixed load document validation function
...
BEGIN
    ...
    SELECT l.company_id INTO target_company_id
    FROM loads l
