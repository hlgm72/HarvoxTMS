...
-- The function was excluding assigned loads, causing zero earnings

CREATE OR REPLACE FUNCTION public.calculate_driver_payment_period_with_validation(target_calculation_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE dpc.id = target_calculation_id;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Cálculo de conductor no encontrado: %', target_calculation_id;
  END IF;
