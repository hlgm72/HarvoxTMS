-- Quinta ronda final: Arreglando las tablas restantes más importantes
...
-- 5. USER_INVITATIONS - Invitaciones de usuarios
...
FOR SELECT
TO service_role
USING (require_authenticated_user() AND (invited_email = (SELECT email FROM auth.users WHERE id = auth.uid())));
...
TO service_role
USING (require_authenticated_user() AND (invited_email = (SELECT email FROM auth.users WHERE id = auth.uid())))
WITH CHECK (require_authenticated_user() AND (invited_email = (SELECT email FROM auth.users WHERE id = auth.uid())));
