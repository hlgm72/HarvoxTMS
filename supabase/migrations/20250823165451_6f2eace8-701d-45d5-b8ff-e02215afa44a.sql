-- Drop the existing function and recreate it with fixed ambiguous column references
...
CREATE OR REPLACE FUNCTION public.create_or_update_company_with_validation(company_data jsonb, company_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF operation_type = 'UPDATE' THEN
    ...
    -- Use table alias 'ucr' to qualify company_id column and avoid ambiguity with parameter
    IF NOT is_superadmin AND NOT EXISTS (
      SELECT 1 FROM user_company_roles ucr
      WHERE ucr.user_id = current_user_id
      AND ucr.company_id = create_or_update_company_with_validation.company_id
    ...
    IF NOT EXISTS (SELECT 1 FROM companies c WHERE c.id = create_or_update_company_with_validation.company_id) THEN
  ...
  IF EXISTS (
    ...
    AND (create_or_update_company_with_validation.company_id IS NULL OR c.id != create_or_update_company_with_validation.company_id)
  ) THEN
  ...
  ELSE
    UPDATE companies c SET
      ...
      updated_at = now()
    WHERE c.id = create_or_update_company_with_validation.company_id
    RETURNING * INTO result_company;
  END IF;
