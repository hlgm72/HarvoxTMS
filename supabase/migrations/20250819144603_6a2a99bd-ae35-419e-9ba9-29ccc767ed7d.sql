-- Fix the create_or_update_load_document_with_validation function to use proper company access
CREATE OR REPLACE FUNCTION public.create_or_update_load_document_with_validation(document_data jsonb, document_id uuid DEFAULT NULL::uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ================================
  -- 1. VALIDATE PERMISSIONS - Use the same logic as can_access_load
  -- ================================
  -- Check if user has access to this load (same company, driver, or creator)
  IF NOT can_access_load(target_load_id) THEN
  ...
  IF operation_type = 'UPDATE' THEN
    IF NOT EXISTS (
      ...
      AND ld.load_id = target_load_id -- Ensure document belongs to the load
    ) THEN
