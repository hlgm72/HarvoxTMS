-- Fix simple_load_operation_with_deductions function to remove updated_by references
...
BEGIN
  ...
  IF has_driver AND (load_data->>'driver_user_id')::UUID IS NOT NULL THEN
    ...
    -- Get owner operator data for the driver
    SELECT * INTO driver_owner_operator
    FROM owner_operators oo
    WHERE oo.user_id = (load_data->>'driver_user_id')::UUID
    ...
    -- Similar logic for dispatching and leasing deductions...
