-- Fix the simple_load_operation_with_deductions function to remove non-existent customer_rate column reference
...
BEGIN
  ...
  IF array_length(stops_array, 1) > 0 THEN
    ...
    IF operation_type = 'UPDATE' THEN
      ...
      RAISE NOTICE '🚨 RPC DEBUG: Existing stops cleared for update';
    END IF;
