-- ULTIMATE SECURITY FIX: Force removal of all SECURITY DEFINER dependencies
-- Using CASCADE to remove all dependent objects

-- ===== 1. DROP EVERYTHING WITH CASCADE =====
DROP FUNCTION IF EXISTS public.user_has_company_access(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.can_access_financial_data(uuid) CASCADE;

-- ===== 2. CREATE CLEAN VIEWS WITHOUT ANY SECURITY DEFINER FUNCTIONS =====
...
COMMENT ON VIEW public.companies_public IS 'SECURED: Company public data view - inherits security from companies table RLS policies. Clean implementation without SECURITY DEFINER functions.';
COMMENT ON VIEW public.companies_financial IS 'SECURED: Company financial data view - inherits security from companies table RLS policies. Clean implementation without SECURITY DEFINER functions.';
COMMENT ON VIEW public.equipment_status_summary IS 'SECURED: Equipment status view - inherits security from company_equipment table RLS policies. Clean implementation without SECURITY DEFINER functions.';