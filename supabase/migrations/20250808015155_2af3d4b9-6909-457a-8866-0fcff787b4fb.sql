-- Drop the existing function first, then recreate it without ambiguous column references
DROP FUNCTION IF EXISTS public.generate_company_payment_periods_with_calculations(uuid,date,date,boolean);
    ...
    IF run_calculations THEN
      ...
      LOOP
        ...
        IF NOT EXISTS (
          ...
          -- Get the period ID with proper alias
