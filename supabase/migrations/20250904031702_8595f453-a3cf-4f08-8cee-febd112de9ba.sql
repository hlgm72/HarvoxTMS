-- Fix fuel expenses calculation to include all relevant statuses
CREATE OR REPLACE FUNCTION public.calculate_driver_payment_period_with_validation(target_driver_period_calculation_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE dpc.id = target_driver_period_calculation_id;
  ...
  WHERE ei.payment_period_id = target_driver_period_calculation_id
  ...
  UPDATE driver_period_calculations SET
    ...
    END,
    updated_at = now()
  WHERE id = target_driver_period_calculation_id;

  -- Return success result
  RETURN jsonb_build_object(
    'success', true,
    'calculation_id', target_driver_period_calculation_id,
